...
pfr_stats_def_filtered_cb %>% 
  ggplot(.,
         aes(x=yds_cmp,
             y=rat,
             alpha=if_else(tm %in% c('PIT', 'LV'), 'PIT/LV', 'Other'))) + 
  geom_point(color=pfr_stats_def_filtered_cb$team_color, size=3.5) +
  geom_text_repel(aes(label = player),
                  familiy='Roboto', fontface='bold', size=3.5) + 
  labs(y='QB rating allowed',
       x='Yards per completion') + 
  scale_alpha_manual(values = c('PIT/LV' = 1,
                                'Other' = .2)) + 
  nfl_analytics_theme() + 
  theme(legend.position = 'none')
```
...
airyards_accu <- airyards_accu %>%
  filter(passer %in% c("L.Jackson", "D.Watson", "J.Fields", "J.Burrow"))
```
