...
week_plot <- play_by_play_2024 %>% summarise(max(week)) %>% pull()
```
...
(epa_per_play_plot <- df_team_epa %>% 
  ggplot(.,
         aes(x=avg_offensive_epa,
             y=avg_defensive_epa)) + 
  # copied code from The F5 -- I want to look at a simpler
  ...
  geom_richtext(aes(x = .20, y = .20, label = "Good O, Bad D"), angle = -1 * rotation, size = 2.75, fontface = 'bold', color = 'black', fill = "#f7f7f7") +
  geom_richtext(aes(x = -.20, y = -.20, label = "Bad O, Good D"), angle = -1 * rotation,  size = 2.75, fontface = 'bold', color = 'black', fill = "#f7f7f7") +
  geom_richtext(aes(x = -.205, y = .205, label = "Bad O, Bad D"), angle = -1 * rotation, size = 2.75, fontface = 'bold', color = 'white',  fill = "#762a83", label.colour = 'black') +
  geom_richtext(aes(x = .215, y = -.215, label = "Good O, Good D"), angle = -1 * rotation,  size = 2.75, fontface = 'bold', color = 'white', fill = "#1b7837", label.colour = 'black') +
  ...
  labs(
    ...
    annotate(geom = 'text', x = .235, y = -.2, label = "**Expected Points Added per Play**: Offense vs. Defense", angle = -1 * rotation, vjust = -3.5, fontface = 'bold', size = 4) +
  annotate(geom = 'text', x = .235, y = -.2, label = glue::glue('2024 season, through Week {week_plot}'), angle = -1 * rotation, vjust = -2.5, size = 3) +
  nfl_analytics_theme() + 
  theme(legend.position = 'none',
        plot.margin = margin(1.15, .5, .5, -.25, unit = 'in')))
        # plot.margin = margin(t = 70, r = 70, b = 70, l = 70))) # Adjust these values as needed
```

```{r fig.height=6, fig.width=6, warning=FALSE}
png(".//images/epa_diamond_plot.png", res = 300, width = 6, height = 6, units = 'in', bg = '#f7f7f7')

print(epa_per_play_plot, vp=viewport(angle=rotation,  
                                     width = unit(6, "in"), 
                                     height = unit(6, "in")))
dev.off()
```
...
```{r fig.height=6, fig.width=6, warning=FALSE}
# Create a rotated viewport and draw the plot
grid.newpage()
pushViewport(viewport(angle = 45))
print(epa_per_play_plot, newpage = FALSE)
popViewport()
```


```{r}
# ggsave('.//images/epa_diamond_plot.png', dpi = 300, width = 6, height = 6, units = 'in')
```
...
(defensive_epa_runpass <- play_by_play_2024 %>% 
  ...
  ggtitle('**Average Defensive Expected Points Added**: Run vs. Pass',
          subtitle = glue::glue('2024 season (through Week {week_plot})')) + 
  nfl_analytics_theme() + 
  theme(legend.position = 'none'))
```
