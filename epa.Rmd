...
play_by_play_2024 %>% 
  ...
  gt_color_pills(sum_yac_epa, digits = 3) %>% 
  gt_color_pills(plays, digits = 0) %>% 
  gt_color_pills(epa_per_play, digits = 3) %>% 
  tab_header(
    title = "Advanced Passing Stats: 2024 Season",
    subtitle = "Sorted by Expected Points Added (EPA) per play. Source: `nflreadr::` package."
  ) %>% 
  gt_theme_athletic() %>%
  cols_width(everything() ~ px(120)) %>% 
  gt_border_bars_top("#1c2632", bar_height = 5) %>%
  gt_border_bars_bottom("#1c2632", bar_height = 25, text = "Inspiration: Georgios Karamanis' Tidy Tuesday work", text_weight = "normal", text_size = 12)
